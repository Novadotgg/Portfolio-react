...
function PaperCard({ paper, index }: { paper: typeof papers[0]; index: number }) {
  ...
  return (
    <div
      ref={ref}
      className={`group relative scroll-reveal ${isVisible ? 'revealed' : ''} stagger-${index + 1}`}
    >
      <div className="glass-card shadow-2xl">
        <CardContent className="p-8 md:p-10">
          <div className="flex flex-col md:flex-row gap-8 items-start">
            <div className="p-5 bg-emerald-500/10 rounded-2xl transition-colors shrink-0 group-hover:bg-emerald-500/20">
            ...
            <div className="flex-1 space-y-6">
              <div className="space-y-4">
                ...
                <h3 className="text-2xl md:text-3xl font-bold text-foreground leading-tight transition-colors group-hover:text-emerald-400">
                  {paper.title}
                </h3>
              ...
              <Button
                asChild
                className="btn-shine bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl px-6 py-3 h-auto text-sm md:text-base font-semibold group/btn relative overflow-hidden transition-all hover:shadow-[0_0_20px_rgba(16,185,129,0.3)]"
              >
